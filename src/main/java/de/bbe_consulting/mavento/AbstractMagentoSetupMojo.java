...
/**
 * Abstract class for mojos that want to setup a Magento instance
...
public abstract class AbstractMagentoSetupMojo extends AbstractMagentoSqlMojo {
     ...
     * @parameter expression="${magento.artifact.group.id}" default-value="com.varien"
     */
     ...
     * @parameter expression="${magento.artifact.is.custom}" default-value="false"
     */
     ...
     * @parameter expression="${magento.use.https.frontend}" default-value="true"
     */
     ...
     * @parameter expression="${magento.dev.log.file}" default-value="system.log"
     */
    protected String magentoDevLogFile;
    
    /** 
     * Logfile name for exceptions, saved to var/log or /tmp if the former is not accessible.<br/>
     * @parameter expression="${magento.dev.log.file.exception}" default-value="exception.log"
     */
    protected String magentoDevLogFileException;
    
    /** 
     * Allow symlinked templates? (only Magento >=1.5.1.0) true|false<br/> 
     * @parameter expression="${magento.dev.allow.symlinks}" default-value="true"
     */
     ...
     * @parameter expression="${magento.extensions.prefered.stability}" default-value="beta"
     */
    protected String magentoExtensionsPreferedStability;
    
    /** 
     * List of core extension keys you want installed after setup.<br/>
     * <pre>Interface_Frontend_Default_Blank,etc</pre>
     * You can append the version string.<br/>
     * <pre>Interface_Frontend_Default_Blank-1.4.0.1</pre>
     ...
     * <pre>Locale_Mage_community_de_DE,market_ready_germany</pre>
     * You can append the version string.<br/>
     * <pre>Locale_Mage_community_de_DE-1.4.0.1</pre>
     ...
     * @parameter expression="${magento.admin.email}" default-value="heinzi@floppel.net"
     */
    protected String magentoAdminEmail;
    
    /** 
     * Admin user first name.<br/>
     * @parameter expression="${magento.admin.name.first}" default-value="Heinzi"
     */
    protected String magentoAdminNameFirst;
    
    /** 
     * Admin user last name.<br/>
     * @parameter expression="${magento.admin.name.last}" default-value="Floppel"
     */
     ...
     * @parameter expression="${magento.backend.frontend.name}" default-value="admin"
     */
     ...
     * @parameter expression="${magento.cache.collections}" default-value="false"
     */
     ...
     * @parameter expression="${magento.remote.addr.header1}" default-value="HTTP_X_REAL_IP"
     */
    protected String magentoRemoteAddrHeader1;
    
    /** 
     * Reverse proxy header2 (local.xml)<br/>
     * @parameter expression="${magento.remote.addr.header2}" default-value="HTTP_X_FORWARDED_FOR"
     */
     ...
     * @parameter expression="${magento.sessiondata.location}" default-value="files"
     */
     ...
     * @parameter expression="${magento.session.cache.backend}" default-value="files"
     */
    protected String magentoSessionCacheBackend;
    
    /** 
     * file|apc|memcached<br/>
     * @parameter expression="${magento.session.cache.slow.backend}" default-value="files"
     */
    protected String magentoSessionCacheSlowBackend;
    
    /** 
     * @parameter expression="${magento.session.cache.slow.backend.store.data}" default-value="false"
     */
    protected Boolean magentoSessionCacheSlowBackendStoreData;
    
    /** 
     * @parameter expression="${magento.session.cache.auto.refresh.fast.cache}" default-value="false"
     */
    protected Boolean magentoSessionCacheAutoRefreshFastCache;
    
    /** 
     * Memcached url.<br/>
     * @parameter expression="${magento.session.cache.memcached.host}" default-value=""
     */
    protected String magentoSessionCacheMemcachedHost;
    
    /** 
     * Memcached port.<br/>
     * @parameter expression="${magento.session.cache.memcached.port}" default-value=""
     */
    protected String magentoSessionCacheMemcachedPort;
    
    /** 
     * @parameter expression="${magento.session.cache.memcached.weight}" default-value=""
     */
    protected String magentoSessionCacheMemcachedWeight;
    
    /** 
     * @parameter expression="${magento.session.cache.memcached.timeout}" default-value=""
     */
    protected String magentoSessionCacheMemcachedTimeout;

    /** 
     * @parameter expression="${magento.session.cache.memcached.interval}" default-value=""
     */
    protected String magentoSessionCacheMemcachedInterval;

    /** 
     * @parameter expression="${magento.session.cache.memcached.status}" default-value=""
     */
    protected String magentoSessionCacheMemcachedStatus;
    
    /** 
     * @parameter expression="${magento.session.cache.memcached.persistent}" default-value=""
     */
    protected String magentoSessionCacheMemcachedPersistent;
    
    /** 
     * @parameter expression="${magento.session.cache.memcached.compression}" default-value="false"
     */
    protected Boolean magentoSessionCacheMemcachedCompression;
    
    /** 
     * @parameter expression="${magento.session.cache.memcached.cachedir}" default-value=""
     */
    protected String magentoSessionCacheMemcachedCachedir;
    
    /**  
     * @parameter expression="${magento.session.cache.memcached.hashed.dir.umask}" default-value=""
     */
    protected String magentoSessionCacheMemcachedHashedDirUmask;
    
    /** 
     * @parameter expression="${magento.session.cache.memcached.file.prefix}" default-value=""
     */
    ...
    /** 
     * @parameter expression="${magento.session.cache.memcached.hashed.dir.level}" default-value=""
     */
    ...
    protected void createFinalSqlDump() throws MojoExecutionException {
		...
		try {
			sqlDumpOrg = FileUtils.fileRead(new File(tempDir+"/sql/"+sqlDumpFileName));
		} catch (IOException e) {
        ...
        {
            throw new MojoExecutionException( "Error creating file " + sqlDumpFiltered, e );
        }
        finally
        {
            if ( w != null )
    ...
    protected void createLocalXml() throws MojoExecutionException {
		...
		if (magentoInstallDate == null || magentoInstallDate.isEmpty()) {
			SimpleDateFormat format = new SimpleDateFormat("EEE, d MMM yyyy HH:mm:ss Z", new Locale("en", "EN"));
			magentoInstallDate = format.format(new Date());
		}
		
		Path localXmlPath = Paths.get(tempDir+"/app/etc/local.xml");
		if (Files.notExists(localXmlPath)) {
			Path localXmlPathTemp = Paths.get(tempDir+"/app/etc/local.xml.template");
		...
		Map<String, String> localTags = getLocalXmlTagMap();
		
		Document localXml = MagentoXmlUtil.readXmlFile(tempDir+"/app/etc/local.xml");
		Document localXmlAdditional = MagentoXmlUtil.readXmlFile(tempDir+"/app/etc/local.xml.additional");
		
		if (magentoSessiondataSavepath != null && !magentoSessiondataSavepath.isEmpty()) {
			// find cache nodes in local.xml.additional
			NodeList cacheNodes = localXmlAdditional.getElementsByTagName("session_save_path");
			// import them to a new node
			Node n = localXml.importNode(cacheNodes.item(0), true);
			// find global node of local.xml
			NodeList globalNodes = localXml.getElementsByTagName("global");
			// append imported nodes to local.xml
			Node g = globalNodes.item(0);			
			g.appendChild(n);
		}
		
		if (magentoSessionCacheLimiter != null && !magentoSessionCacheLimiter.isEmpty()) {
			// find cache nodes in local.xml.additional
			NodeList cacheNodes = localXmlAdditional.getElementsByTagName("session_cache_limiter");
			// import them to a new node
			Node n = localXml.importNode(cacheNodes.item(0), true);
			// find global node of local.xml
			NodeList globalNodes = localXml.getElementsByTagName("global");
		...
		if (magentoSessionCacheBackend.equals("memcached") || magentoSessionCacheBackend.equals("apc") || magentoSessionCacheBackend.equals("xcache")) {
			// find cache nodes in local.xml.additional
			NodeList cacheNodes = localXmlAdditional.getElementsByTagName("cache");
			// import them to a new node
			Node n = localXml.importNode(cacheNodes.item(0), true);
			// find global node of local.xml
			NodeList globalNodes = localXml.getElementsByTagName("global");
		...
		if (mVersion.getMajorVersion() <= 1 && mVersion.getMinorVersion() < 6) {
			if (magentoRemoteAddrHeader1 != null && !magentoRemoteAddrHeader1.isEmpty()) {
				// find cache nodes in local.xml.additional
				NodeList cacheNodes = localXmlAdditional.getElementsByTagName("remote_addr_headers");
				// import them to a new node
				Node n = localXml.importNode(cacheNodes.item(0), true);
				// find global node of local.xml
				NodeList globalNodes = localXml.getElementsByTagName("global");
    ...
    protected void updateLocalXml() throws MojoExecutionException {
    	...
    	String localXmlPath = tempDir+"/app/etc/local.xml";
		Document localXml = MagentoXmlUtil.readXmlFile(localXmlPath);
		
		MagentoXmlUtil.updateDbValues(magentoDbHost, magentoDbUser, magentoDbPasswd, magentoDbName, localXml);
		
	    try {
	    	MagentoXmlUtil.writeXmlFile(MagentoXmlUtil.transformXmlToString(localXml), localXmlPath);
		} catch (TransformerException e) {
			throw new MojoExecutionException(e.getMessage(), e);
		}
    }
    
    protected NodeList findXmlNode(Object obj, String xpathExpression) throws XPathExpressionException {
    ...
    protected void createOldCacheConfig() throws MojoExecutionException {
		// get params for regex replace in local.xml
		Map<String, String> cacheTags = getOldCacheTagMap();
		
		String cacheConfigTemplate = "a:7:{s:6:\"config\";i:@CACHE_CONFIG@;s:6:\"layout\";i:@CACHE_LAYOUT@;s:10:\"block_html\";i:@CACHE_BLOCK@;s:9:\"translate\";i:@CACHE_TRANSLATE@;s:11:\"collections\";i:@CACHE_COLLECTIONS@;s:3:\"eav\";i:@CACHE_EAV@;s:10:\"config_api\";i:@CACHE_API@;}";

		String finalCacheConfig = null;
		finalCacheConfig = MagentoUtil.replaceTags(cacheConfigTemplate, cacheTags);
		
		File cacheConfigFiltered = new File(tempDir+"/app/etc/use_cache.ser");
    ...
    protected void setupMagento() throws MojoFailureException, MojoExecutionException {
    	...
    	if (magentoUrlBaseHttps != null && !magentoUrlBaseHttps.isEmpty()) {
    		magentoUrlBaseHttps = MagentoUtil.validateBaseUrl(magentoUrlBaseHttps, true);
    	} else {
		...
		// create db
		MagentoSqlUtil.createMagentoDb(magentoDbUser, magentoDbPasswd, magentoDbHost, magentoDbPort, magentoDbName, getLog());   	
        ...
        MagentoSqlUtil.importSqlDump(dumpFileName, magentoDbUser, magentoDbPasswd, magentoDbHost, magentoDbPort, magentoDbName, getLog());
        ...
        if (magentoArtifactIsCustom) {
			...
			if (magentoExposeExceptions && (mVersion.getMajorVersion() == 1 && mVersion.getMinorVersion() > 3)) {
				...
				try {
					...
					throw new MojoExecutionException("Error while enabling exception printing! " + e.getMessage(), e);
				}
				getLog().info("..done.");
			}
			...
			if (magentoUseSampleData && !magentoArtifactIsCustom) {
				Path magentoSourcePath = Paths.get(tempDir+"/mavento_setup/sample_data");
				Path magentoTargetPath = Paths.get(tempDir);
				try {
					getLog().info("Copying sample data..");
					CopyFilesVisitor cv = new CopyFilesVisitor(magentoSourcePath, magentoTargetPath, false);
        ...
        if ( magentoExtensionsOther != null && !magentoExtensionsOther.isEmpty() ) {
        	if ( magentoExtensionsOther.equals("*") ) {
        		Path eDir = Paths.get("extensions");
        		Path tDir = Paths.get(tempDir);
        		try {
					Files.createDirectories(eDir);
					ExtractZipVisitor ev = new ExtractZipVisitor(tDir, getLog());
	        	...
	        	String[] k = magentoExtensionsOther.split(",");
	        	for (String extensionKey : k) {
	        		String fPath = "extensions/"+extensionKey;
        ...
        if (!isIntegrationTest) {
	        ...
	        try {
				...
				throw new MojoExecutionException("Error while copying to: "+magentoTargetPath.toAbsolutePath() + " " + e.getMessage(), e);
			}
			getLog().info("..done.");
        }
		...
		// add any core_config_data properties after extension install, as the extension might overwrite them else
    ...
    private void setupPear() throws MojoExecutionException {
		...
		if (magentoPearEnabled) {
			File magentoTargetPath = new File(targetDir);
			...
			MagentoUtil.executePearCommand(pearExecutable.getAbsolutePath(), new String[] {"mage-setup"}, magentoTargetPath.getAbsolutePath(), mVersion, getLog());
			getLog().info("..done.");
			
			if (magentoPearUpgrade || (mVersion.getMajorVersion() == 1 &&
										mVersion.getMinorVersion() == 4 &&
										mVersion.getRevisionVersion() == 2)) {
			
				getLog().info("Updating pear channel..");
				MagentoUtil.executePearCommand(pearExecutable.getAbsolutePath(), new String[] {"channel-update", "pear.php.net"}, magentoTargetPath.getAbsolutePath(), mVersion, getLog());
				if (mVersion.getMajorVersion() <= 1 && mVersion.getMinorVersion() < 6) { 
					getLog().info("Upgrading pear..");
					MagentoUtil.executePearCommand(pearExecutable.getAbsolutePath(), new String[] {"upgrade", "--force", "PEAR"}, magentoTargetPath.getAbsolutePath(), mVersion, getLog());
				}
				getLog().info("..done.");
			}
    ...
    protected void installPearModules(String channel, String[] extensionKeys, File pearExecutable) throws MojoExecutionException  {
 			...
 			if (!extensionKey.isEmpty()) {
 				...
 				if (mVersion.getMajorVersion() == 1 && mVersion.getMinorVersion() <= 4) {
 					...
 					if (extensionKey.contains("-")) {
 						String[] s = extensionKey.split("-");
 						String extensionVersion = "";
 						if (s.length == 2) {
 							extensionKey = s[0];
 							extensionVersion = s[1];
 						}
 						getLog().info("Installing magento-"+channel+"/"+extensionKey+"-"+extensionVersion+"..");
				...
				MagentoUtil.executePearCommand(pearExecutable.getAbsolutePath(), params, targetDir, mVersion, getLog());
				getLog().info("..done.");
 			}
		}
    }
    ...
    private Map<String, String> getSqlTagMap() {
    	Map<String, String> tokenMap = getSqlTagMapBasic();
...
//		tokenMap.put("auto_refresh_fast_cache", magentoSessionCacheAutoRefreshFastCache ? "1" : "0");
//		tokenMap.put("slow_backend_store_data", magentoSessionCacheSlowBackendStoreData ? "1" : "0");
...
//		tokenMap.put("compression", magentoSessionCacheMemcachedCompression ? "1" : "0");
//		tokenMap.put("cache_dir", magentoSessionCacheMemcachedCachedir);
//		tokenMap.put("hashed_directory_level", magentoSessionCacheMemcachedHashedDirLevel);
//		tokenMap.put("hashed_directory_umask", magentoSessionCacheMemcachedHashedDirUmask);
//		tokenMap.put("file_name_prefix", magentoSessionCacheMemcachedFilePrefix);

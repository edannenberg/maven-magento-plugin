...
public class MagentoSetupTestMojo extends AbstractMagentoSetupMojo {
     ...
     * @parameter expression="${php.dependencies.target.dir}" default-value="${project.build.directory}/php-deps"
     * @required
     */
    ...
    /** 
     * Magento instances for integration tests are kept in the project's build directory.<br/>
     * The plugin will symlink the Magento instance to this location, usually a htdocs/vhost directory.<br/>
     ...
     * @parameter expression="${magento.test.artifact.group.id}" default-value="com.varien"
     */
    protected String magentoTestArtifactGroupId;
    
    /** 
     * ArtifactId of a custom Magento artifact for integration tests.<br/>
     * @parameter expression="${magento.test.artifact.id}" default-value="magento"
     */
     ...
     * true: dump is supplied by magento artifact in $artifactroot/mavento_setup/sql
     * @parameter expression="${magento.test.db.dump.supplied.by.artifact}" default-value="false"
     */
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		List<String> l = session.getGoals();
		if (l.get(0).equals("eclipse:eclipse") || magentoTestIgnore ) {
			return;
		}
		
		if ( magentoTestDbName == null || magentoTestDbPasswd == null || magentoTestDbUser == null || magentoTestRootLink == null || magentoTestUrlBase == null) {
			throw new MojoExecutionException("Error missing properties magento.test.db.* ,magento.test.root.link or magento.test.url.base." +
		...
		Path setupMarker = Paths.get(buildDirectory.getAbsolutePath()+"/maven-magento-plugin/"+Paths.get(phpDependenciesTargetDir).getFileName()+"_setup");
		if (Files.notExists(setupMarker)) {
	       ...
	       setupMagento();
			...
			Path magentoSource = Paths.get(tempDir);
			Path magentoTarget = Paths.get(magentoTestRootLink);
			try {
				if (Files.isSymbolicLink(magentoTarget)) {
					Files.delete(magentoTarget);
				}
				if (Files.exists(magentoTarget, LinkOption.NOFOLLOW_LINKS)) {
					throw new MojoExecutionException("Could not relink to: "+magentoTarget+" ,check it's either a symlink or non existing.");
				}
		...
		Path moduleSource = Paths.get(project.getBasedir().getAbsolutePath()+"/src/main/php");
		if (Files.exists(moduleSource)) {
			Path moduleTarget = Paths.get(tempDir);
			CopyFilesVisitor crv = new CopyFilesVisitor(moduleSource, moduleTarget, false);
			try {
				Files.walkFileTree(moduleSource, crv);
			} catch (IOException e) {
				throw new MojoExecutionException("Error copying module source to: "+moduleTarget+" "+e.getMessage(), e);
			}
		}
		...
		try {
			URL magentoUrl = new URL(magentoTestUrlBase+"index.php/admin");
	        HttpURLConnection mc;
			mc = (HttpURLConnection) magentoUrl.openConnection();

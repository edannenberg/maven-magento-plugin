...
package de.bbe_consulting.mavento.ui;
...
import org.apache.maven.archetype.ui.ArchetypeSelectionQueryer;
...
/** @plexus.component */
public class MagentoArchetypeSelector extends AbstractLogEnabled implements ArchetypeSelector {
        ...
        ArchetypeSelectionFailure
    {
        ...
        if ( definition.isDefined() )
        {
            ...
            if ( found != null )
            {
                ...
                getLogger().info( archetypeMode + " repository missing. Using the one from " + archetype
                                      + " found in catalog " + catalogKey );
            }
            else
            {
                getLogger().warn( archetypeMode + " not found in any catalog. Falling back to central repository (http://repo1.maven.org/maven2)." );
        ...
        else if ( definition.isPartiallyDefined() )
        {
            ...
            if ( found != null )
            {
                ...
                getLogger().info( archetypeMode + " " + archetype + " found in catalog " + catalogKey );
            }
            else
            {
                getLogger().warn( "Specified " + archetypeMode.toLowerCase() + " not found." );
        ...
        if ( !definition.isPartiallyDefined() )
        {
            ...
            {
            	if ("snippet".equals(props.getProperty("magentoArchetypeIdentifier"))) {
            		getLogger().info( "No snippet defined. Using " + DEFAULT_SNIPPET_ARTIFACTID + " ("
		                    + definition.getGroupId() + ":" + DEFAULT_SNIPPET_ARTIFACTID + ":" + definition.getVersion()
		                    + ")" );
            ...
            if ( !definition.isPartiallyDefined() )
            {
                throw new ArchetypeSelectionFailure( "No valid " + archetypeMode.toLowerCase() + "s could be found to choose." );
            }
        }
    ...
    private Map<String, List<Archetype>> getArchetypesByCatalog( String catalogs )
    {
        ...
        for ( String catalog : StringUtils.split( catalogs, "," ) )
        {
            ...
            {
            	Iterator<Archetype> ite = archetypeManager.getDefaultLocalCatalog().getArchetypes().iterator();
            	List<Archetype> localArchetypes = new LinkedList<Archetype>();
            	while (ite.hasNext()) {
					Archetype a = ite.next();
            		if (a.getArtifactId().toLowerCase().contains("magento") && a.getArtifactId().toLowerCase().contains(props.getProperty("magentoArchetypeIdentifier"))) {
            			localArchetypes.add(a);
            		}
				}
            	getLogger().info("Found "+localArchetypes.size()+" Magento " + archetypeMode + "s in repository " + catalog);
            ...
            else if ( "remote".equalsIgnoreCase( catalog ) )
            {
                ...
                if ( archetypesFromRemote.size() > 0 )
                {
                	...
                	getLogger().info("Found "+remoteArchetypes.size()+" Magento " + archetypeMode + "s in repository " + catalog);
                    archetypes.put( "remote", remoteArchetypes );
                }
                else
                {
                    getLogger().warn( "No " + archetypeMode.toLowerCase() + " found in remote catalog. Defaulting to internal catalog" );
                    archetypes.put( "internal", archetypeManager.getInternalCatalog().getArchetypes() );
                }
            }
            ...
            {
            	Iterator<Archetype> ite = archetypeManager.getRemoteCatalog( catalog ).getArchetypes().iterator();
            	...
            	getLogger().info("Found "+httpArchetypes.size()+" Magento " + archetypeMode + "s in repository " + catalog);

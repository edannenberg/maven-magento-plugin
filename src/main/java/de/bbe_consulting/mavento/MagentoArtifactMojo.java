...
/**
 * Create a dependency artifact from a running Magento instance. This goal does
 * not need an active Maven project.<br/>
 * 
 * <pre>
...
public class MagentoArtifactMojo extends AbstractMojo {
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        final CopyFilesVisitor cv = new CopyFilesVisitor(Paths.get(magentoPath), tmpDir, false);
        try {
        ...
        final String dumpFile = Paths.get(tmpDir.toAbsolutePath().toString()
                + "/mavento_setup/sql/magento.sql").toAbsolutePath().toString();
        ...
        if (!magentoKeepDbSettings) {
            ...
            final Path tmpLocalXml = Paths.get(tmpDir.toAbsolutePath().toString() + "/app/etc/local.xml");
            ...
            try {
                MagentoXmlUtil.writeXmlFile(
                        ...
                        tmpLocalXml.toAbsolutePath().toString());
        ...
        getLog().info("mvn install:install-file -Dpackaging=jar -Dfile="
                        + jarFileName + " -Dversion=" + defaultVersion
